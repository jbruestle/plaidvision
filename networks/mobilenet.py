...
# MobileNet thinks it only works with TensorFlow, but the truth is that it
...
# by monkeypatching the backend() function during network initialization.
...
def classify(img, top_n=5):
    ...
    data = mobilenet.preprocess_input(data)
    predictions = model.predict(data)
    return mobilenet.decode_predictions(predictions, top=top_n)[0]
